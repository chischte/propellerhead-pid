int get_sensor_values()
{
  ...
  //If the Motorarm is horizontal, the ACC_GRAVITY_RAW has 0G
  //If the motorarm is vertical, it has 1G
  //*****************************************************************************
  //*****************************************************************************
  //*****************************************************************************

  float ACC_GRAVITY_MAX_G_VALUE = 8500; //MANUALLY WRITE THE SENSOR_VALUE WHEN SENSOR MEASURES FULL G-FORCE
  //ACC_GRAVITY_RAW = ACC_GRAVITY_RAW * .1; //SCALE DOWN TO PREVENT INT OVERFLOATING
  ...
  {
    if (ACC_GRAVITY_RAW != 0) //ZEROS CAN BE CAUSED BY SENSOR ERROR
    {
      int ACC_SMOOTHCOUNTER = 3; //USE THIS VALUE TO ADAPT SMOOTHING
      ACC_GRAVITY_LPF = (ACC_GRAVITY_LPF * (ACC_SMOOTHCOUNTER - 1) + ACC_GRAVITY_RAW) / ACC_SMOOTHCOUNTER;
    }
  }
  //CONVERT ACCELERATOR VALUES TO ANGLE // unit [°/10] to avoid float
  //*****************************************************************************
  GRAVITY_ANGLE_LPF_NEW = asin(ACC_GRAVITY_LPF / (ACC_GRAVITY_MAX_G_VALUE)) * -65; //last factor and addend are experimentally determined
  ...
  //*****************************************************************************
  //GYRO_RAW = GYRO_RAW + 30; //TO MINIMIZE MEASURED PERMANENT SENSOR ERROR
  ...
  GYRO_ANGULAR_SPEED_SMOOTHED = (GYRO_ANGULAR_SPEED_SMOOTHED * (GYRO_SMOOTHCOUNTER - 1) + GYRO_ANGULAR_SPEED) / GYRO_SMOOTHCOUNTER;
  //*/
  ...
  GYRO_ANGLE = GYRO_ANGLE + (GYRO_ANGULAR_SPEED * (transmission_delta_t * 0.000001));
  ...
  GYRO_ANGLE_CALIBRATED = (GYRO_ANGLE * (GYRO_CALIBRATION_SMOOTHVALUE - 1) + (GRAVITY_ANGLE_LPF)) / GYRO_CALIBRATION_SMOOTHVALUE;
  GYRO_ANGLE = GYRO_ANGLE_CALIBRATED; //UPDATE THE ANGLEVALUE FOR THE NEXT LOOP
