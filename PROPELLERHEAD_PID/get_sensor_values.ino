void GetSensorValues()
{
   ...
   if (accGravityRaw == accGravityRaw) // FILTER OUT NaN's - ONLY NaN's DON'T PASS THIS TEST!
  {
    if (accGravityRaw != 0) // zeros can be caused by sensor errors
    {
      ...
      accGravityLpf = (accGravityLpf * (acc_smoothcounter - 1)
          + accGravityRaw) / acc_smoothcounter;
    }
  }
  // CONVERT ACCELERATOR VALUES TO ANGLE [°]
    gravityAngleLpfNew = asin(accGravityLpf / (acc_gravity_max_g_value))
  ...
  gyroAngularSpeedSmoothed = (gyroAngularSpeedSmoothed
      * (gyro_smoothcounter - 1) + gyroAngularSpeed) / gyro_smoothcounter;

  // CALCULATING ANGLE: NEW_ANGLE = OLD_ANGLE + (ANGULAR_SPEED*DELTA_TIME)
  transmissionDeltaT = micros() - transmissionStopwatch;
  gyroAngle = gyroAngle
      + (gyroAngularSpeed * (transmissionDeltaT * 0.000001));
  ...
  gyroAngleCalibrated = (gyroAngle * (gyro_calibration_smoothvalue - 1)
      + (gravityAngleLpf)) / gyro_calibration_smoothvalue;
